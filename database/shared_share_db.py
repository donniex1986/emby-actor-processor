        ...
        with conn.cursor() as cur:
            cur.execute(
                ...
                ON CONFLICT(channel_id) DO UPDATE SET
                    ...
                    file_count=CASE WHEN COALESCE(EXCLUDED.file_count, 0) > 0 THEN EXCLUDED.file_count ELSE shared_completed_season_share_channels.file_count END,
                    total_size=CASE WHEN COALESCE(EXCLUDED.total_size, 0) > 0 THEN EXCLUDED.total_size ELSE shared_completed_season_share_channels.total_size END,
                    status=EXCLUDED.status,
