...
def normalize_lang_code(lang_str: str, lang_mapping: Optional[List[Dict[str, Any]]] = None) -> str:
    ...
    if lang_str in ['guo', 'guoyu', 'chs', 'zh-cn', 'zh-sg', 'zh-hans', 'cmn', 'mandarin', 'taiwan', '台湾', '台灣', '国语', '普通话', '中文', '简体', '简中']:
        return 'chi'
...
def parse_full_asset_details(item_details: dict, id_to_parent_map: dict = None, library_guid: str = None) -> dict:
    ...
    emby_primary_source = emby_media_sources[0] if emby_media_sources else None
...
def is_movie_subscribable(movie_id: int, api_key: str, config: dict) -> bool:
    ...
    delay_days = int(mp_config.get('delay_subscription_days', 0))
