...
# --- 补齐共享资源所需的 115 指纹 ---
def task_repair_p115_fingerprints(processor):
    """
    扫描所有在库电影/分集视频，补齐共享资源模块依赖的 115 PC 和 SHA1。

    核心修复能力已下沉到 p115_fingerprint_helpers.repair_p115_fingerprints_for_rows，
    这样 check_season_consistency 可以只针对当前季复用，不需要反向导入 media，
    避免 helpers <-> media 导入循环。
    """
    task_name = "补齐115指纹"
    logger.info(f"--- 开始执行 {task_name} 任务 ---")
    task_manager.update_status_from_thread(0, "正在扫描缺失 PC/SHA1 的在库视频...")
    ...
    try:
        with connection.get_db_connection() as conn:
            ...
            cursor.execute("""
                ...
                ORDER BY item_type, title NULLS LAST, tmdb_id
            """)
            rows = [dict(row) for row in cursor.fetchall()]
    except Exception as e:
        logger.error(f"  ➜ [{task_name}] 读取媒体库资产失败: {e}", exc_info=True)
        task_manager.update_status_from_thread(-1, "读取媒体库资产失败")
        return

    if not rows:
        logger.info(f"  ➜ [{task_name}] 没有可处理的在库电影/分集资产。")
        task_manager.update_status_from_thread(100, "没有可处理的在库视频")
        return
    ...
    try:
        ...
        stats = repair_p115_fingerprints_for_rows(
            ...
            should_stop=processor.is_stop_requested,
            progress_callback=task_manager.update_status_from_thread,
            progress_interval=50,
        )
    except Exception as e:
        logger.error(f"  ➜ [{task_name}] 执行指纹补齐失败: {e}", exc_info=True)
        task_manager.update_status_from_thread(-1, f"{task_name}失败: {e}")
        return

    final_msg = (
        f"{task_name}完成：扫描资产 {stats.get('scanned_assets', 0)}，缺失 {stats.get('missing_assets', 0)}，"
        f"成功补齐 {stats.get('fixed_assets', 0)}，缓存命中 {stats.get('cache_fixed_assets', 0)}，"
        f"现场查询 {stats.get('api_fixed_assets', 0)}，写回媒体项 {stats.get('updated_rows', 0)}，"
        f"回写115缓存 {stats.get('cache_updates', 0)}，失败 {stats.get('failed_assets', 0)}。"
    )
    logger.info(f"  ➜ {final_msg}")
    task_manager.update_status_from_thread(100, final_msg)
