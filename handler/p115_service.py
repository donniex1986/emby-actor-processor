    ...
    def get_client(cls):
        ...
        class StrictSplitClient:
            ...
            def fs_mkdir(self, name, pid):
                ...
                1. 先查 DB 缓存
                2. API 创建
                3. 如果 API 返回“已存在”，不切备用接口，直接回查同级目录并写缓存
                """
                parent_cid = str(pid)
...
class SmartOrganizer(P115MediaAnalyzerMixin):
    ...
    def get_target_cid(self, ignore_memory=False, season_num=None):
        ...
        def _memory_cid_matches_current_file(check_cid, current_ext):
            if not check_cid:
                return False
            for r in self.rules:
                if str(r.get('cid')) != str(check_cid) or not r.get('enabled', True):
                    continue
                rule_exts = {
                    str(ext).strip().lower().lstrip('.')
                    for ext in r.get('file_extensions') or []
                    if str(ext).strip()
                }
                return not rule_exts or (current_ext and current_ext in rule_exts)
            return False
            ...
            try:
                ...
                with get_db_connection() as conn:
                    with conn.cursor() as cursor:
                        cursor.execute(
                            """
                            SELECT id, name, sha1, size, washing_snapshot_json
                            FROM p115_filesystem_cache
                            WHERE parent_id = %s
                            """,
                            (str(batch_target_cid),),
                        )
                        ...
                        if self.media_type == 'tv':
                            ...
                            if target_eps:
                                cursor.execute(
                                    """
                                    SELECT por.file_id, por.original_name, por.renamed_name, por.season_number,
                                           pfc.sha1, pfc.size, pfc.washing_snapshot_json
                                    FROM p115_organize_records por
                                    LEFT JOIN p115_filesystem_cache pfc ON pfc.id = por.file_id
                                    WHERE por.tmdb_id = %s
                                      AND por.media_type = 'tv'
                                      AND por.status = 'success'
                                      AND por.season_number = ANY(%s)
                                      AND por.file_id IS NOT NULL
                                    """,
